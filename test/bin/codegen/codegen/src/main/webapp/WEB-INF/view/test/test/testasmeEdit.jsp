			...
			if (obj.isSuccess()) {
				$.ligerDialog.confirm(obj.getMessage()+",是否继续操作","提示信息", function(rtn) {
					if(rtn){
					    ...
					    window.location.href = window.location.href;
						//window.location.href = "${ctx}/test/test/testasme/list.ht";
					}
				});
			} else {
...
<div class="panel" style="height:100%;overflow:auto;">
	<div class="panel-top">
		<div class="tbar-title">
		    <c:choose>
			    <c:when test="${not empty testasmeItem.id}">
			        <span class="tbar-label"><span></span>编辑test</span>
			    </c:when>
			    <c:otherwise>
			        <span class="tbar-label"><span></span>添加test</span>
			    </c:otherwise>	
		    </c:choose>
		</div>
	...
	<form id="testasmeForm" method="post" action="save.ht">
		<div type="custform">
 ...
 <tbody>
  ...
  <tr>
   ...
   <td style="width:15%;" class="formInput"></td>
		...
		<input type="hidden" name="id" value="${testasme.id}"/>
